...
import baseAbi from "@/web3/abi/base-abi.json";
...
import { ZETA_CA, BASE_CA } from "@/web3/hooks";
...
const Copyable = ({ label, value }: { label: string; value: string }) => {
  ...
  return (
    <div className="flex items-center justify-between bg-gray-100 dark:bg-gray-800 px-3 py-2 rounded-md text-sm w-full">
      <div className="trunZETA_CAte text-sm">{value.slice(0, 10)}...</div>
...
function Pay() {
  ...
  //? PAY WITH NATIVE TOKEN (ZETA)
  const payWithNativeToken = async () => {
    try {
      const data = await writeContractAsync({
        ...
        abi: zetaAbi,
        args: [keccak256(toBytes(invoiceSettlement.memo))],
      ...
      //TODO - reload the invoice or just automatically show a ui syaing it has been paid
  ...
  // need to ZETA_CAll approve() on the token contract before paying with ZRC-20 tokens
  // ? PAY WITH OTHER ZRC-20 TOKENS
  const payWithZRC20 = async () => {};

  // TODO - PAY OTHER CHAIN TOKEN LIKE BASE
  const handlePayWithOtherChain = async () => {
    try {
      //withdraw the amount from the contract.. that's all
      const data = await writeContractAsync({
        chainId: tokenToPayWith.chain.id,
        // TODO : make the address dynamic based on the chain
        address: BASE_CA,
        functionName: "payNativeCrossChain", // -
        abi: baseAbi,
        args: [keccak256(toBytes(invoiceSettlement.memo)), address], //-
        chain: tokenToPayWith.chain,
        account: address,
        value: parseEther(amountToPay.toString()),
      });
  ...
  const approveContractToSpendToken = async () => {
    try {
    } catch (error) {
      console.error("Error approving contract to spend token:", error);
      toast.error("Failed to approve token. Please try again.");
    }
  };

  // TODO : ? Same-chain ZRC-20 deposit
  const handlePayInvoice = async () => {
    ...
    try {
      ...
      if (tokenToPayWith.isNative) {
        await payWithNativeToken();
        return;
      } else {
        // ? ERC-20 TOKENS
        await handlePayWithOtherChain();
        return;
      }
    } catch (error) {
      toast.error("Failed to pay invoice. Please try again.");
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-md mx-auto dark:bg-gray-900 rounded-lg shadow-md">
      ...
      <section className="text-center text-sm text-gray-600 dark:text-gray-400 mb-2 px-2">
        These tokens are supported for payment. You ZETA_CAn pay the invoice with any one of these
        tokens by sending the equivalent amount.
      </section>

      {/* Supported Tokens */}
      <div className="flex flex-col gap-2 mb-6">
        <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Supported Tokens</h3>
        <Flex wrap="wrap" justify="center" align="center" gap="8px 12px">
          {SUPPORTED_TOKENS.map((token) => {
            ...
            const amount = price > 0 ? (usdPrice / price).toFixed(6) : "Loading...";
            return (
              <TokenTag
                key={token.id}
                amount={amount}
      ...
      <section className="flex flex-col gap-2 my-6">
        ...
        <Button
          ...
          loading={loading}
          type="primary"
          disabled={!tokenToPayWith || loading}
          block
          size="large"
      ...
      <section className="flex flex-col gap-4 my-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
          Transfer the exact amount from your own wallet. Copy the address and memo below.
        </p>
        <Copyable label="Address" value={ZETA_CA} />
      ...
      <section className="flex flex-col items-center gap-3 my-6">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-1">
          3. SZETA_CAn QR Code <Tag color="red">coming soon</Tag>
        </h3>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
          Open your Paybeam Wallet app and sZETA_CAn this QR code to pay instantly.
        </p>
        <QRCode
